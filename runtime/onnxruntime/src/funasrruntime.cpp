					...
					for(int i=0; i<msg_stamp.size()-1; i+=2){
						if(i+1 < msg_stamp.size()){
							try{
								...
								LOG(ERROR) << "Invalid timestamp format: " << e.what();
							}catch(const std::out_of_range& e){
								LOG(ERROR) << "Timestamp out of range: " << e.what();
							}catch(const std::exception& e){
								LOG(ERROR) << "Error parsing timestamp: " << e.what();
							}catch(...){
								LOG(ERROR) << "Unknown error parsing timestamp";
							}
						}
					}
				}
